...
public class ProjectRoleIsPMOrQCLeader extends AbstractWebCondition {
    @Override
    public boolean shouldDisplay(final ApplicationUser applicationUser, final JiraHelper jiraHelper) {
        ...
        final Collection<ProjectRole> projectRoles = prm.getProjectRoles(applicationUser,
                                                                         Helper.getCurrentProject(jiraHelper));
        if (projectRoles.size() > 0) {
            for (final ProjectRole projectRole : projectRoles) {
                if (Constants.ROLE_PM.equals(projectRole.getName()) ||
                    Constants.ROLE_QC_LEADER.equals(projectRole.getName())) {
                    return true;
                }
            }
        }
